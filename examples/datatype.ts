...
if (!chOrm) throw new Error("Failed to initialize ClickhouseOrm");
const doDemo = async () => {
  // create database 'orm_test'
  await chOrm.createDatabase();

  await chOrm.client.query(`DROP TABLE IF EXISTS ${db.name}.${tableSchema.tableName}`).toPromise();

  // register schema and create [if] table
  const tableModel = await chOrm.model(tableSchema);

  const uuidRes: any = await chOrm.client.query(`SELECT generateUUIDv4() as uuid`).toPromise();
